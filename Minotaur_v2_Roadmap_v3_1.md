# 🗺️ ROADMAP — Minotaur v2 (v3.1)

**Version 3.1 • August 10, 2026 • Forward-only file — history lives in `Minotaur_v2_Ledger_v1_0.md`**

*(v3.1, Aug 10: housekeeping close — WF-1a pilot LIVE at `d5637e2`; CLAUDE.md relocated `4a1ee1b`; Arch Prompt v28 installed; Next-Sessions + Status refreshed. v3.0 = the migration delivery, same session.)*
...
- **Cowork pilot (next):** seed `~/Developer/Minotaur-Cowork/`, then run 2–3 document-production sessions in Cowork to test the edit-and-diff-review flow; keep, fix, or drop it after review.
...
Live at `https://minotaur.app`. HEAD **`d5637e2`** — **SPRINT 3 COMPLETE** (`1055e76`) and **housekeeping COMPLETE** (`cc96e82` fold → `4a1ee1b` relocation → `d5637e2` **WF-1a pilot LIVE**, its own maiden phrase-gated commit). CLAUDE.md = 70KB standing law + 20 arch-record files + the Guarded Architectures Index. Suite **1250/89** · frozen **3/3 vs `be0769de`** · migrations unchanged (`l0_r1_…` last) · fixture pristine. Visual gate → docket **GATE-VIS**. Governing set current: Arch Prompt v28 · Strategy v14 · Roadmap v3.1 + Ledger v1.0. **Next action:** seed COWORK-P2 → the Sprint-4 planning session (the map written once into the docket below).
