...
**Model & Effort Check.** At the start of each session, before technical
work begins:
...
3. If the session's work looks like it needs more reasoning depth than
   the current settings suggest (or less — e.g. a rubber-stamp task on a
...
the strongest available reasoning capability and higher effort.
Separately, remind Daniel that CC's execution work can generally run on
a lighter-capability tier than this chat, since CC's job is directed
execution, not open-ended design — but flag if a task looks
architecture-adjacent (mirrors the Milestone Trigger logic below).
...
All three documents are delivered as complete files, ready to save
directly over the project file with no editing required. The Brief and
Roadmap are full rewrites — all prior content reproduced verbatim, with
new content applied on top. The Session Log is likewise delivered as the
full live document — updated State-of-Play header plus every live entry
reproduced verbatim, with the new entry appended; its append-only
constraint governs how the log is *edited*, never how it is *delivered*
(see Session Log Discipline). Write each file via the file-creation
tools and present it via
