...
| **MMM-CAT** | The category band on the Equipment List can be edited only when the list is sorted by category. Ruled, never built; you hit it live on 2026-09-08. | **CC** · needs nothing |
...
|---|---|---|
| **CHANGEALL-EMPTYFIND** | With a find that matches nothing, the Change All button stays lit and the press does nothing — no dialog, no message, no write (`changeAllArmed` latches). Third instance of the dead-control shape in one unit. | The next cable-list unit |
...
| **TAB-AFTER-SCOPE** | Daniel, 2026-09-24: after the "just this one or all" dialog closes, Tab leaves the grid for the browser's own chrome instead of the next cell. Measured by CC, unfixed. | The next equipment-list unit |
| **LENGTH-FALLBACK** | An invalid length draft reverts to the stored length, and a Change All then offers that across the found set — the same silent-fallback shape as the four names, which are fixed. | Rides the next Change All work |
| **METHOD-CREATE** | Should typing a method that does not exist CREATE it, as v1's value list may? Registered while fixing the refusal; unmeasured against v1. | Daniel's ruling, with the methods admin |
...
| **PARALLEL-CLOCK-STAMP** | Two parallel openers were stamped ~3h ahead of the real clock by the main line; two sessions caught it independently. PARALLEL-CLOCK means the real time of writing. | The next close |
...
| **INSPECTOR-ARM** | Change All did nothing from a Method field with no row selected, and the recorded cause does not match the code. **Closed by CHANGEALL-MMMCAT, pending its commit** — measured 2026-09-22: the surface was the CABLE list, where four inspector fields stayed live with no row selected. | Arch, measurement first |
...
*Roadmap v4.50 · 2026-09-24 · retires v4.49. Written under Architecture Prompt v39. NOW + NEXT + THE SEQUENCE ≤ 8,000 characters is the only hard cap. Code `d25d366` live. Migration ledger **83**, unchanged — no migration this session; `supabase/migrations/` holds 22 files, 72–83 all with a file of record. Suite 3,848 / 208 at the last green run; frozen 3/3; visual 28/28 after one declared frame-01 event; lint exit 0 (263 warnings). **Two deploys: Group Parts and Device Patch `d12b80d`, the device-join fix `d25d366`.** CHANGEALL-MMMCAT is built, gated 6 of 7 and NOT committed.*
