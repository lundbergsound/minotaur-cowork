...
The hard-stop list above is deliberately the complement of arch's
Milestone Triggers: CC may act autonomously on exactly the surface arch
does *not* halt for, and must stop on exactly the surface arch *does*. If
a project needs a stricter hard-stop (e.g. a specific fragile component),
add it to that project's copy — never loosen the shared four.
...
log reaches **6 entries**, set the header's Rollover field to
...
2. Generate the archive file — the **oldest 4** live entries, frozen,
