...
**In flight: NOTHING. No unit is running.** The last unit closed and pushed is
**EQUIP-LIST-PARITY (`c525a55`)**, 2026-09-07 — the equipment list and its cover
letter print at v1's full size, and the estimate paginator stops losing rows. Its
predecessor was CAT-NOTE-RULE (`f4192cd`).

Counts MEASURED at that close, 2026-09-07, every one re-run rather than carried
over: suite **2827 tests across 149 test files, ZERO skipped** · frozen **3/3 vs
`be0769de`** · build clean · strict typecheck clean · dependency gate clean ·
visual gate **28/28 at 0.0000%** on every run · migration ledger **57**,
UNCHANGED for five consecutive units — last
`20260901043028_changeall_foundset_bulk_write_rpcs`, Arch-applied 2026-09-01 from
the Cowork seat. **Production at `c525a55` CONFIRMED READY** by
`node scripts/vercel-preopen.mjs`, run by Daniel 2026-09-08.
...
**Next unit: ROADMAP-REFACTOR carrying CLAUDEMD-ATLIMIT — Arch's own, no CC
handoff. Then FIELD-CONTRACT, then DARKMODE-PASS.** ⚠ **NO BUILD UNIT RAN BEFORE
IT**, because this file had three characters of headroom and the next unit could
not have written its first finding. Diagnoses, measurements and open items live in
the Roadmap Docket and the Session Log, not here.

⚠ **THE HAZARD THIS UNIT LEFT LIVE, and the thing to watch for on the next real
print:** the equipment list loses ZERO blocks across seven configurations against
a control at `f4192cd` that loses **42**, and the cover letter's own pages
overflow by zero — but the proof is on seeded throwaways, and a real show is what
finds a shape the seven do not cover. **A page ending mid-category with the next
opening on a `(continued)` header is normal; a page whose last row is cut off in
