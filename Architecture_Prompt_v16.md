...
Each document update is delivered as a downloadable file, named exactly
to match its destination project file. Each file is a full rewrite of
the entire document — all prior content reproduced verbatim, with new
content appended or applied on top. Do not produce diffs, partial
files, or append-only excerpts. The file is intended to replace the
project file in one step, so it must be complete and self-contained.
