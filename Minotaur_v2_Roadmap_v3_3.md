# 🗺️ ROADMAP — Minotaur v2 (v3.3)

**Version 3.3 • August 10, 2026 • Forward-only file — history lives in `Minotaur_v2_Ledger_v1_0.md`**

*(v3.3, Aug 10: the S4-1 decks-clear close — GATE-VIS · DWV · P9-DROP · VERCEL-CLI all EXIT to the Ledger (Aug-10 S4-1 appends); migration ledger 39→40; **COWORK-P2 AMENDED to "B with seatbelt"** — P2a rollover shakedown → S4-2 runs IN COWORK same sitting if clean → COWORK-REV at S4-2 close, default EXPAND; TOK-VC registered; Cowork Prompt v1 authored. v3.2 = the Sprint-4 planning close, same day.)*
...
- **Next: the Cowork pilot opens (Daniel's desk, Cowork Prompt v1).** First act = **P2a, the Session Log rollover** — it's due anyway (the log hit 10 entries) and doubles as the shakedown: cold-start from the governing docs, the mounts, byte-exact file delivery. **If P2a comes out clean, S4-2 (S4-AUTO) runs IN COWORK in the same sitting** — the autocomplete conversion, designed and CC-handoffs authored from the Cowork seat; migrations (if any surface) route back to the claude.ai Arch seat.
- **At S4-2's close: COWORK-REV** — the keep/expand/revert review, **default EXPAND toward all-Arch-work-in-Cowork unless the pilot registers a named defect.**
- **Then the box unit family opens** — the sprint's core: Box List → Box Types → Contents/Assignments → the three box PDFs. **P2b (the Boxes evidence digest from the full local v1 corpus) lands before S4-3's design gate.** Your printer/show packets (GATE-PDF · OL25WR · C4) adjudicate as they arrive without blocking the open.
- **At your desk, independent of sessions:** the R8 backup drill before the September attorney engagement; the three-PDF measurement packet + OL25WR die-cut re-gate + C4 pagination confirm whenever you're at a printer/show; the TOK-VC token rotation at convenience.

---

## Current Status (August 10, 2026)

Live at `https://minotaur.app`. HEAD **`1426e03`** — **S4-1 DECKS-CLEAR COMPLETE** (Aug 10, all four items exited to the Ledger): **GATE-VIS** PASS (ON RERUN, Daniel-accepted; 16/16 @ 0.0000% quiet-machine) · **DWV** zero dead sites (class CLOSED, no commit) · **P9-DROP** migration **40** applied + types regen `281958f` · **VERCEL-CLI** live `1426e03` (REST script + CLAUDE.md standing pre-open line; the screenshot step retired). Suite **1250/89** · frozen **3/3 vs `be0769de`** · migration ledger **40** (`p9_step3_drop_phantom_model_from_v_cable_list` last) · fixture pristine SELECT-proven. Governing set: Arch Prompt v28 · Strategy v14 · Roadmap v3.3 + Ledger v1.0 (Aug-10 S4-1 appends) · **Cowork Prompt v1**. Session Log at 10 entries — **rollover DUE = Cowork P2a**. **Next action:** Daniel installs the close package, opens Cowork on Prompt v1 (P2a → S4-2 same sitting if clean).

## 🏗 Sprint Plan (scope of record)

**Sprints 0–3: CLOSED** — exited to the Ledger (Sprint 3 closed Aug 7–9 with S3-G under the Aug-7 compression). **Sprint 4 — Boxes + the basket** (spec-complete `abeab54`): **the map of record, written ONCE at the Aug-10 planning session (Daniel-confirmed as presented):** **S4-1** decks-clear — **COMPLETE Aug 10** (all four exits in the Ledger) → **S4-2** S4-AUTO (one idiom, one declared baseline event — before any box surface, forced by the day-one constraint; OQ #3's method field rides here by ruling; **runs IN COWORK per the B-with-seatbelt ruling, P2a shakedown first; COWORK-REV at its close**) → **S4-3…n** the box unit family (Box List → Box Types → Contents/Assignments → three PDFs; GATE-PDF/OL25WR/C4 adjudicate as Daniel's packets arrive, non-blocking; **P2b Boxes digest before the S4-3 design gate**) → **S4-mid** D-2b + BASE-EVT as the sanctioned frames session · FOCUS-MP in between-units headroom → **S4-late** GS-C (SPL-C rides here) → LINE-STECK (+STECK-COLOR) → S3-G-2 (+UNFLAG-WARN riding its print work) → ALLOC → **S4-close** EXPORT+GBD deadline pair (EXPORT carries PORT-COV's export half) → WF1A-SUNSET + auto-mode-rider review → KKK-PASS immediately after close. **PORT-CRIT ruled (Aug 10):** ports wait for the box family, then ONE coverage unit (PORT-COV) extends import+export to equipment+cable+box; the five-leg **PORT-READY** criterion is named in the Ledger. Standing sprint constraints carry: box views responsive from the start · method/box autocomplete-from-day-one · auto-mode rider EXPIRES at Sprint 4 close. **Post-Sprint-4 / post-beta:** GCM completion · Maintenance · Spreadsheet Export · RF/Walkies/Circuits · IAS Frequency Import · real-time collaboration · licensing build (~Nov design) · PWA/offline (v3 candidate) — sequenced when their triggers approach.
...
| COWORK-P2a | Cowork pilot maiden act + shakedown: the Session Log rollover (DUE — log at 10 entries; archive oldest 8 byte-exact → `~/Developer/Minotaur-Cowork/session-log-archives/`, retain 2) | GATED | Daniel | First Cowork act, before any normal work |
| COWORK-P2b | Boxes evidence digest from the full LOCAL v1 corpus (DDR + behavioral catalogs — the capability the claude.ai seat lacks); feeds the box family's design gate | OPEN | Daniel | Before S4-3's design gate |
| COWORK-REV | Keep/expand/revert review of the Cowork pilot — **default EXPAND toward all-Arch-work absent a named defect** (burden reversed, Daniel-ruled) | GATED | Daniel | S4-2 close |
| TOK-VC | Vercel token hygiene: rotate the chat-pasted pilot token (revoke + re-mint dashboard→terminal) · 90-day expiry renewal | OPEN | Daniel | At convenience · renewal ~early November |
| PORT-COV | Import/export coverage extension: v1 cable + box data parsing IN (cable/tail/bundle/lines — never parsed in v1; DDR extraction net-new) + cable/box in the export v1-leg OUT. PORT-CRIT ruling Aug 10; ports open after, per PORT-READY leg 5 (Ledger) | OPEN | Arch | After the box family; merged into the EXPORT slot |
| D-2b | Bundle Clone/Ext linkage (evidence banked incl. d2b_01–03 `3ffd371`; s441/s704/s504 frames; the B6 duplicate-cascade gap; Trigger B linkage schema) | OPEN | Arch | S4-mid, cable-side leg 1 |
| S504 | Residual s504 delete-variant frames (3 of 4); gates only D-2b's gate detail | BLOCKED | Daniel | When v1 cooperates |
| GS-C | Cable completeness: SPL-C split (661-wart adjudication) · INS-1 inspector tail/line summary · (bbbb) Show-Me routing · candidate (qqq)/(sss) CTF Enter hardening | OPEN | Arch | S4-late, cable-side leg 2 (the cable split rides here) |
| S3-G-2 | Residual v1 outputs (Shop Orders Basic/Detailed + remaining reports) — Sprint-4 home Daniel-CONFIRMED Aug 9 | OPEN | Arch | S4-late, after LINE-STECK |
| FOCUS-MP | Focus/nav mini-pass: (lll) ⌥⇧I on uncommitted model · (nnn) band-click currentRowId · (jjjj) fresh-commit ⌥⇧I → Category · (kkkk) F4-No blink · Aug-7 notes #4–#9 | OPEN | Arch | S4 between-units headroom |
| UNFLAG-WARN | Unflagged-cables print warning | OPEN | Arch | Rides S3-G-2's print work |
...
| LINE-STECK | Line/steck label unit — ports the deferred line-row 🖨 trio (606/607/608, evidenced) | OPEN | Arch | S4-late, after GS-C |
| STECK-COLOR | Steck-color verify (P11 rider) | OPEN | Arch | Rides LINE-STECK |
| CMDK | ⌥⇧K command palette + top-bar shortcut icon (chord-registry verify pre-design; palette contents Parity-governed, opens with its own ruling block) | OPEN | Arch | Post-Sprint-4 (declined from Sprint 4, Aug 10) |
...
| EXPORT | v2→v1 export unit revival: Pass D v1-leg + second dry-run (EXP-6 L1) + the S-12 chain re-align + carries PORT-COV's export half (Aug-10 ruling) | OPEN | Arch | End of Sprint 4 |
...
| M0-WIN | Method-0 windowing equivalent check (gates PORT-READY leg 2, the "equipment-list PDF renders" criterion) | OPEN | Arch | Pre-port |
| 3TL | Three-tier library architecture (Minotaur global · user default · project) | OPEN | Arch | Pre-beta, unscheduled |
| S4-AUTO | Sprint-4 autocomplete conversion event: equipment method + box fields → autocomplete text · box field editable · (iii) filter pulldowns → autocomplete · the (mmm) method/box/group band-editability half — one idiom, one baseline event | OPEN | Arch | S4-2 — before any box surface (OQ #3 rides here, ruled Aug 10); **runs IN COWORK per B-with-seatbelt** |
| BASE-EVT | Declared baseline-event session: Bundle-List/D-2b frames + the dark cluster (yyy)(rrr)(ttt)(ggg)(xxx) + the KeyR unbind | OPEN | Arch | S4-mid, rides D-2b (sanctioned session) |
| ALLOC | Allocate as type-conversion (D9) — the `allocated_from_id` consumer (landed + fenced consumer-less) | OPEN | Arch | S4-late (the Allocate call) |
...
Resolved questions exit to the Ledger's Rulings with the ruling that resolved them (this migration moved: S3-G-2 home · Aug-25 · fold path · Sprint-4-planning placement · Ledger v1.0 · UX-catalog destination · status vocabulary · bundles UNIQUE · a-3 units consumer · V-1/baseline placements; **the Aug-10 planning session moved: #3 method-field sequencing · port-readiness gate criteria — both to the Ledger's Aug-10 rulings**).
...
*End of Roadmap v3.3 — August 10, 2026. History: `Minotaur_v2_Ledger_v1_0.md`. The previous format's final file, Roadmap v2.46, is archived byte-exact.*
