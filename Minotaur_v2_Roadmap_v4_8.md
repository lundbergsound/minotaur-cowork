# 🗺️ ROADMAP — Minotaur v2 (v4.8)

**Version 4.8 • August 26, 2026 (evening) • Forward-only file — history lives in `Minotaur_v2_Ledger_v1_0.md`**

## 🧭 Next Sessions (RM-6 — plain English, kept current)

- **THREE UNITS CLOSED. PRINT-PARITY unit 1, the box label family, CLOSED at `8c6b2e0`** (`e17fd97..8c6b2e0`, 6 files, +872/−241) — the worst-placed element in the family was **51.35 pt (18.11 mm)** out of position and is now **0.63 pt (0.22 mm)**. **The root cause was fixed at source, not nudged:** the build derived every position as a percentage of a cell computed as **page ÷ grid**, where v1's cell is the **FileMaker layout body**, smaller and inset — so every percentage had the wrong denominator. All three sheets are now absolute point geometry measured off the v1 PDFs. Also: all five style transpositions · the nested-parent **BoldItalic** top slot · the empty location band suppressed · Replacement's **column-major** fill · a box name that v2 was **wrapping** and v1 keeps on one line · `BUNDLE_FLAG` → `#FED1CF`.
- **AND BEFORE IT: 2-MIG-b applied** — the five gap arms, migration ledger **54 → 55**. **2-IMP-b closed at `e17fd97`** (`9a140ca..e17fd97`, 33 files, +4161/−198): `cable.xlsx`, `lines.xlsx` and `tails.xlsx` are mapped sheets now — **162 columns** — and the v1→v2→v1 round trip is **100% on all three corpora**.
...
- **IN FLIGHT: the CABLE-PARITY EVIDENCE parallel session** (`handoffs/parallel/260826-1520_cable-parity-evidence-opener.md`). It writes no repo files and **issues no CC handoffs** — anything needing a build is staged to `handoffs/_staging/` marked DRAFT, NOT ISSUED, and **the MAIN LINE re-anchors and issues it**. **CC is IDLE.**
- **⚠ ITS FIRST TASK REFRAMES THE CABLE BLOCKER RATHER THAN ACCEPTING IT.** The record says the cable family is blocked because the July-11 run is an unreconstructable partial found set (794 of 1,119 cables). **Nobody has asked how many of the seventeen v1 cable outputs are Find-driven at all.** The box family escaped entirely because its found sets are **DATA-DERIVED** — several cable outputs may be too, and would be gateable today against PDFs already on the mount. For the rest: **do not reconstruct July 11 — define a pairing reproducible BY CONSTRUCTION.**
- **NEXT ON THE MAIN LINE: 2-IMP-c** (clone links by recompute, the last unit of leg 2), then **2-MIG-c** (SPLITOF-NO-ARM). Import/export continues while the cable evidence is gathered — the parallel produces paper, not code, so the two do not contend.
...
- **Counts at `8c6b2e0`:** suite **2291 tests across 129 test files, ZERO skipped** · frozen **3/3 vs `be0769de`** · typecheck 0 · dependency gate empty · build clean · visual **INTERMITTENT at GATE-THEMEGLYPH — run 1 26/28, run 2 28/28 on the SAME tree, both recorded, no baseline written.**
- **⚠ COUNT NOTATION (TESTCOUNT):** `2291/129` is **TESTS / TEST FILES**, never passing / skipped.
...
- **⚠ TWO NEW STANDING RULES FROM THE EVENING.** **DEVICE-VM-IS-NOT-CC:** anything Arch measures through the device shell about the ENVIRONMENT — installed packages, versions, paths, OS — does **not** describe CC's machine; `device_bash` runs on Linux (`claude`), CC runs on macOS, and only the FILES are shared. It will recur in every handoff naming a tool. **CLOSE-THEN-CONTINUE:** when work continues past an installed close, it takes its own Session Log entry and the STATE OF PLAY header is brought current — the header is the durable part, the entries are not editable.
- **⚠ THE FILEMAKER INSPECTOR DISAGREED WITH FILEMAKER'S OWN PRINT.** `borderRadius: 3`, cited to the Inspector, is absent from the printed paths. **A rendered STYLE value the print engine ignores is not evidence of what prints; a BEHAVIOURAL rule — does this field shrink at N characters — still is.** Recorded because OQ-3 asks Daniel for an Inspector screenshot.
...
- **Pre-open:** refs from ref files, never git · check for an orphaned `index.lock` · migration ledger **55** · Vercel READY at `8c6b2e0`. **⚠ `CLAUDE.md`'s run fence is STALE — it still names PORT-COV leg 2. Fixing it rides the FIRST handoff of the next session, with the queued `CLAUDE.md` package.** One untracked stray remains at `docs/reference/evidence/equipment/`.
- **Rollover:** clear. Session Log at 7 entries.
...
Live at `https://minotaur.app` at **`8c6b2e0`**, pushed (`9a140ca..e17fd97..8c6b2e0`). Migration ledger **55**. Counts: suite **2291 tests / 129 test files, zero skipped** · frozen **3/3 vs `be0769de`** · typecheck 0 · dependency gate empty · build clean · visual **intermittent at GATE-THEMEGLYPH, recorded as such**. Governing set: **Arch Prompt v32** · Strategy v14 · **Roadmap v4.8** (retiring v4.7) + Ledger v1.0 (Aug-26 append). Session Log at **7 entries, rollover clear.** **In flight:** the cable-parity evidence parallel. **Next on the main line:** 2-IMP-c → 2-MIG-c, then **R8-DRILL, overdue, six days**.
...
| **CABLE-PARITY EVIDENCE — the parallel** | **IN FLIGHT** | evidence only, no repo writes, no CC handoffs · triages the seventeen v1 cable outputs into data-derived vs Find-driven |
| **PORT-COV leg 2 — 2-IMP-c (clone links, by recompute)** | **NEXT ON THE MAIN LINE** | carries CLONE-TOKEN and the `mult_line` link vocabulary; depends on R-2 |
